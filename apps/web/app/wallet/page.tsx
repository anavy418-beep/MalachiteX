...
export default function WalletPage() {
  ...
  const loadWallet = useCallback(async (reason: "mount" | "retry" | "refresh" = "mount") => {
    ...
    try {
      ...
      const errorMeta = err as Error & { status?: number; url?: string; rawMessage?: string };
      const rawMessage = errorMeta.rawMessage ?? (err instanceof Error ? err.message : String(err ?? ""));
      const isAuthFailure =
        errorMeta.status === 401 ||
        errorMeta.status === 403 ||
        /unauthorized|forbidden|invalid token|jwt/i.test(rawMessage);

      if (isAuthFailure) {
        if (process.env.NODE_ENV !== "production") {
          console.warn("[wallet] auth-like wallet response; preserving current session state", {
            reason,
            endpoint: walletEndpoint,
        ...
        setError("Unable to verify live wallet data right now. Showing safe wallet state.");
        return;
      }
      ...
      {error ? (
        <Card className="border-amber-500/30 bg-amber-950/20">
          <CardContent className="pt-6">
            <p className="text-sm text-amber-200">Unable to load live wallet data. Showing safe wallet state.</p>
